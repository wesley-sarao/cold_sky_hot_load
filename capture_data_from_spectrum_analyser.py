...
This script uses the Rohde & Schwarz FSU-26 spectrum analyser to carry out
...
Last update reason: Update Ghana script for Michael to use on the
    Wideband OMT project
...
f_center = 5750  # Measurement center frequency in MHz
f_bw = 2500  # Measurement frequency BW in MHz
...
RefLev = -40  # Reference level of spectrum analyser in dBm
Atten = 5  # Spectrum analyser attenuation in dB. A minimum of 5 dB
sweeps = 20  # Number of sweeps to capture for each measurement

# Test settings and notes
# -----------------------------------------------------------------------------
band = 'wideband pol rear'  # directory that the data will be saved to
meas = 'proper test in courtyard'  # file name. Specify a meaninful name
...
t_hot_C = 15.0  # Measured hot load temperature in degree C (ambient load)
t_hot_K = t_hot_C + 274.15
sky_conditions = 'clear'  # it is good practice to record this
wind = 'none'
comment = ''  # Any additional comments. Perhaps, the test intention
...
def sa_save_hot_trace(local_file_name,
    ...
    trace_dbm = np.zeros((sweeps_local+1, f_points_local+1), dtype=float)
    # array for storing phot data
    p_meas_w = np.zeros((sweeps_local+1, f_points_local+1), dtype=float)
...
# Open the spectrum analyser
sa = rm.open_resource('TCPIP0::192.168.2.124::5025::SOCKET')
...
# Read the start freq
f_start_set = double(sa.query('FREQ:STAR?'))
# Read back the stop freq
f_stop_set = double(sa.query('FREQ:STOP?'))
# Read back the number of sweep points
f_points_set = double(sa.query('SWE:POIN?'))
...
def plot_T_eff():
    ...
    plt.figure(2)
    plt.clf()
    plt.plot(T_eff[0, data_s:data_f],
